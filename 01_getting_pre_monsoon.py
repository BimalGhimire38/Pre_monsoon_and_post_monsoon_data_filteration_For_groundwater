...
may_columns = [col for col in range(1, len(headers) + 1) if headers[col - 1] and (
    "-05-" in str(headers[col - 1]))]

# Create a new workbook for filtered data
new_wb = Workbook()
new_ws = new_wb.active
new_ws.title="May_Data"

# Copy headers (keeping necessary columns)
for idx, col in enumerate([1, 2, 3, 4] + may_columns, start=1):  # Keep first 4 columns (District, Station Name, X, Y)
    new_ws.cell(row=1, column=idx, value=ws.cell(row=1, column=col).value)

# Copy data for May 
for row in range(2, ws.max_row + 1):
    for idx, col in enumerate([1, 2, 3, 4] + may_columns, start=1):
        new_ws.cell(row=row, column=idx, value=ws.cell(row=row, column=col).value)

# Save the filtered data
filtered_file = "01_May_Data.xlsx"
